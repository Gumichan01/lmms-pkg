...
/**
    ...
    ```
    A valid project is a song project generatred by a supported version of LMMS.
*/
        ...
        if ( root != nullptr )
        {
            ...
            if ( root_name == ROOT_NAME )
            {
                ...
                if ( project_type == PROJECT_TYPE_VALUE )
                {
                    ...
                    {
                        std::cerr << "XML: This project was generated by a not supported version of LMMS: "
                                  << version << "only one of the following versions are supported: " //<< valid_versions
                                  << ".\n";
                    }
                }
                else
                {
                    std::cerr << "XML: Invalid project type. It must be a song, not '"<< project_type << "'\n";
                }
            }
            else
            {
                std::cerr << "XML: Invalid root name: " << root_name << ". Expected: " << ROOT_NAME << "\n";
            }
        }
        else
        {
            std::cerr << "XML: Invalid XML file\n";
        }
    }
    else
    {
